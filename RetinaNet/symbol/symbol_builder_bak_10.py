...
def get_symbol_train(network, num_classes, num_layers, from_layers, num_filters, strides, pads,
    ...
    """
    current_network = import_module(network)
    ...
    conv_feat = current_network.get_resnet_conv(data, num_layers, **kwargs)
    _, conv_fpn_feat = current_network.get_resnet_conv_down(conv_feat)
    conv_fpn_feat.reverse()    # [P3, P4, P5, P6, P7]

    loc_preds, cls_preds, pn_preds, anchor_boxes = multibox_layer(conv_fpn_feat, \
    ...
    pn_prob_ = mx.symbol.SoftmaxActivation(pn_preds, mode="channel")
    pn_prob = mx.sym.Custom(pn_preds, pn_prob_, cls_target, op_type="focal_loss", name="pn_prob",
                             gamma=2.0, alpha=0.25, normalize=True, is_pn=True)
    ...
    loc_loss_ = mx.symbol.smooth_l1(name="loc_loss_", \
        data=loc_target_mask * (loc_preds - loc_target), scalar=0.1)
    ...
    # group output
    out = mx.symbol.Group([cls_prob, pn_prob, loc_loss, cls_label, det])
    return out
