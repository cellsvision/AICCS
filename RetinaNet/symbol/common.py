...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    pn_score_weight = mx.symbol.Variable(name='pn_score_weight',
                                          init=mx.init.Normal(sigma=0.01))
    pn_score_bias = mx.symbol.Variable(name='pn_score_bias',
                                        init=FocalBiasInit(2, 0.01), attr={'__lr_mult__': '1.0'})
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        pn_pred = mx.symbol.Convolution(data=cls_conv4,
                                         weight=pn_score_weight,
                                         bias=pn_score_bias,
                                         kernel=(3,3),
                                         stride=(1,1),
                                         pad=(1,1),
                                         num_filter=num_pn_pred,
                                         name="{}_pn_pred_conv".format(from_name))
        cls_pred = mx.symbol.transpose(cls_pred, axes=(0,2,3,1))
        cls_pred = mx.symbol.Flatten(data=cls_pred)
        cls_pred_layers.append(cls_pred)

        pn_pred = mx.symbol.transpose(pn_pred, axes=(0,2,3,1))
        pn_pred = mx.symbol.Flatten(data=pn_pred)
        pn_pred_layers.append(pn_pred)
    ...
    pn_preds = mx.symbol.Concat(*pn_pred_layers, num_args=len(pn_pred_layers), \
        dim=1)
    pn_preds = mx.symbol.Reshape(data=pn_preds, shape=(0, -1, 2))
    pn_preds = mx.symbol.transpose(pn_preds, axes=(0, 2, 1), name="multibox_pn_pred")
    ...
    return [loc_preds, cls_preds, pn_preds,anchor_boxes]
