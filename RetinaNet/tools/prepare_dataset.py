...
from dataset.mscoco import Coco, Coco2017
from dataset.concat_db import ConcatDB
from dataset.cvs_voc import CVSVoc

def load_cvs(image_set, devkit_path, shuffle=False, class_names=None, true_negative=None):
    """
    ...
    imdbs = []
    for s in image_set:
        imdbs.append(CVSVoc(s, devkit_path, shuffle, is_train=True, class_names=class_names, true_negative_images=true_negative))
    if len(imdbs) > 1:
...
def load_coco2017(image_set, dirname, shuffle=False):
    """
    wrapper function for loading ms coco dataset
    ...
    """
    anno_files = ['instances_' + y.strip() + '.json' for y in image_set.split(',')]
    isets = [y for y in image_set.split(',')]
    assert anno_files, "No image set specified"
    imdbs = []
    for af, iset in zip(anno_files, isets):
        af_path = os.path.join(dirname, 'annotations', af)
        imdbs.append(Coco2017(af_path, iset, dirname, shuffle=shuffle))
    if len(imdbs) > 1:
        return ConcatDB(imdbs, shuffle)
    else:
        return imdbs[0]
...
def parse_args():
    ...
    parser.add_argument('--root', dest='root_path', help='dataset root path',
                        ...
                        type=str)
    parser.add_argument('--shuffle', help='shuffle list',
                        action='store_true')
    parser.add_argument('--true-negative', dest='true_negative', help='use images with no GT as true_negative', action='store_true')
...
if __name__ == '__main__':
    ...
    if args.dataset == 'pascal':
        ...
        db = load_coco2017(args.set, args.root_path, args.shuffle)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    elif args.dataset == 'cvs':
        db = load_cvs(args.set, args.root_path, args.shuffle, args.class_names, args.true_negative)
        print("saving list to disk...")
        db.save_imglist(args.target, root=args.root_path)
    else:
    ...
    cmd_arguments = ["python3", im2rec_path,
                     os.path.abspath(args.target), os.path.abspath(args.root_path),
