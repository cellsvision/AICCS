...
from train.custom_schedule import CosineCyclicSchedule, LinearWarmUp
...
def get_warmup_cosine_cyclic_schedule(lr, lr_step, lr_ratio, num_example, batch_size, begin_epoch, warmup_epoch=1):
    max_lr = lr
    min_lr = int(max_lr * lr_ratio)

    lr_epoch = [int(ep) for ep in lr_step.split(',')]
    assert len(lr_epoch) == 1
    cycle_epoch = lr_epoch[0]
    epoch_size = int(num_example / batch_size)
    cycle_length = cycle_epoch * epoch_size
    start_iter = begin_epoch * epoch_size
    cc_scheduler = CosineCyclicSchedule(min_lr=min_lr, max_lr=max_lr, cycle_length=cycle_length)

    lr_scheduler = LinearWarmUp(cc_scheduler, 0, int(epoch_size*warmup_epoch), start_iter=start_iter)

    return (lr, lr_scheduler)


def get_lr_scheduler(learning_rate, lr_refactor_step, lr_refactor_ratio,
                     num_example, batch_size, begin_epoch, **kwargs):
    """
...
def train_net(network, train_path, num_classes, batch_size,
              ...
              checkpoint_period=1, min_neg_samples=0, warmup_epoch=1):
    """
    ...
    train_iter = DetRecordIter(train_path, batch_size, data_shape, mean_pixels=mean_pixels,
                               label_pad_width=label_pad_width, path_imglist=train_list, is_train=True, max_num_classes=num_classes, **cfg.train)
        ...
        for k, v in arg_dict.items():
            if k in args:
                if v.shape != args[k].shape:
                    ...
                    if not 'pred' in k or not 'score' in k:
                        fixed_param_names.append(k)
                    else:
                        print(k,'would be changed in this training')
    elif pretrained:
    ...
    if optimizer.lower() == 'sgd_cosine':
        get_scheduler = get_warmup_cosine_cyclic_schedule
        optimizer = 'sgd'
    else:
        get_scheduler = get_lr_scheduler

    learning_rate, lr_scheduler = get_scheduler(learning_rate, lr_refactor_step,
                                                   lr_refactor_ratio, num_example, batch_size, begin_epoch, warmup_epoch=warmup_epoch)

    #learning_rate, lr_scheduler = get_lr_scheduler(learning_rate, lr_refactor_step,
    #                                               lr_refactor_ratio, num_example, batch_size, begin_epoch)

    #learning_rate, lr_scheduler = get_warmup_cosine_cyclic_schedule(learning_rate, lr_refactor_step,
    #                                               lr_refactor_ratio, num_example, batch_size, begin_epoch, warmup_epoch=warmup_epoch)
